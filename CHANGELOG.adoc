...
* **[Archive]** Added `PersistentSubscription` to Aeron Archive for all languages (Java, C++ and C).
+
`PersistentSubscription` is an alternative to ReplayMerge with IPC and Spy support, plus a fallback mechanism for slow consumers.
...
* **Breaking:** **[C Driver]** The name resolver interface has been changed. The C name resolver now requires an
additional `on_start` lifecycle method.

* **Breaking:** **[C Driver]** The `aeronmd` binary now requires the aeron client library (`libaeron.so`/`aeron.dll`)
