...
* **[Cluster]** Treat Cluster membership query errors as warnings.
* **[Cluster]** Sort snapshot entries in the `recoding.log` by `logPosition` within the same `leadershipTermId` before
sorting by snapshot entry type.
* **[Cluster]** Provide detailed description for `clusterMembers` list entries and emphasise that `ingress:port` is
public facing.
* **[Cluster]** Optimise sending egress responses, i.e. use `tryClaim` if small and `offer` if message is too big.
* **[Cluster]** Introduce `ClusteredServiceContainer.Context.extendedTerminationHook(ExtendedTerminationHook)`,
`ConsensusModule.Context.extendedTerminationHook(ExtendedTerminationHook)`,
and `ClusterBackup.Context.extendedTerminationHook(ExtendedTerminationHook)`.
* **[Client/C]** Fix `aeron_subscription_resolved_endpoint` signature, i.e. remove const from the out parameter `address`.
(https://github.com/aeron-io/aeron/issues/2130[#2130])
* **[Tracing]** Renaming the concept of `EventLogging` to `Tracing`. (https://github.com/aeron-io/aeron/pull/2131[#2131])
* **[Archive/C]** Add `aeron_archive_persistent_subscription_get_persistent_subscription_context` and
`aeron_archive_persistent_subscription_get_and_own_persistent_subscription_context` to allow the caller to take
ownership of the persistent subscription context, in the same way as `aeron_archive_get_and_own_archive_context` does.
(https://github.com/aeron-io/aeron/pull/2132[#2132])
* **[C]** Upgrade to `CMake` 4.4.3.

== 1.53.0 (2026-08-26)

=== New & Noteworthy

* **Breaking:** **[Client]** Image `controlledPeek` APIs were removed:
**  `io.aeron.Image.controlledPeek` and `io.aeron.Image.position(long)`.
** `aeron_image_controlled_peek` and `aeron_image_set_position`.
** `aeron::Image::controlledPeek` and `aeron::Image::position`.

* **Breaking:** **[Logging]** The `aeron-agent.jar` has been removed so setting `-javaagent:<file name>` is no longer
required. The existing configuration options are still supported.
** The option `aeron.event.log.render.data` allows for the data portion of frame in and frame out messages to be printed.
** Dynamic enablement of logging is no longer supported.
** Some of the logging messages are formatted differently.

* **Breaking:** **[Client/C/C{plus}{plus}]** Surface the specific archive error codes to the C/C++ user API.
** Add new `AERON_ARCHIVE_*_ERROR_CODE` constants and move `ARCHIVE_*_ERROR_CODE` constants to the
`aeron_archive_client.c` file to hide from clients. This may cause compilation errors for clients.
** Add `errorCode()` to C++ exceptions to allow access to the error code from the API in both the transport and
archive clients.
** Add additional error code to user string conversion in the `aeron_error.c` implementation.
** Translate the Archive error codes to the user error codes in the C aeron archive client and async client.

* **Breaking:** **[Archive]** Remove support for `ConnectRequest` (`templateId = 2`) message which allowed
unauthenticated login to Archive. This message was not used by official Archive clients used since
<<1.24.0 (2019-11-24)>>.
+
NOTE: The corresponding logging event `CMD_IN_CONNECT` was also removed.

* Introduce more distinct Aeron-specific thread names across all modules/components,
configurable via the new `aeron.thread.naming` system property:
+
--
** `classic`(default) - use old names (e.g. `sender`, `receiver`, `conductor`)
** `new` - use new standardized names (e.g. `aeron-md-snd`, `aeron-md-rcv`, `aeron-md-cnd`)
--

* **[Driver]** https://github.com/aeron-io/aeron/wiki/Channel-Configuration#timestamps[Outgoing and incoming timestamps]
(`media-rcv-ts-offset`, `channel-rcv-ts-offset`,`channel-snd-ts-offset`) are now applied to all messages within a packet
sent/received not just the first message.
...
* **[Driver/C]** Fix CPU affinity assignment for native resource agent.
* **[Driver/C]** Start agents in reverse order, i.e. ensure that no threads share CPU affinity with the `conductor`
thread.
* **[Driver]** Sender work count should be based on actual work done, i.e. bytes sent/received and driver tasks
executed. (https://github.com/aeron-io/aeron/issues/2092/[#2092])
* **[Driver/Java]** Receiver triggering address re-resolution should not count towards work done.
* **[Driver/C]** Receiver work count calculation aligned with Java side.
* **[Driver/C]** Use `IP_MULTICAST_ALL` or `IPV6_MULTICAST_ALL` socket options to avoid receiving data from other
multicast addresses with the same port.
* **[Client/Java]** Access cnc file with read only access for `LogInspector` and `DriverTool`.
* **[Cluster]** Do not set `AeronArchive.Context.errorHandler()` in order to propagate exceptions directly to the
`ConsensusModuleAgent/ClusteredServiceAgent`.
* **[Cluster]** Handle terminal exceptions in a single place, check if local Archive connection is still alive in error
path and simplify `Election` error handling.
* **[Driver/Java]** Use plain access when accessing own counters and `getAcquire` when accessing counters owned by other
threads.
* **[Archive/Java]** Map `EMPTY_RECORDING` in `ArchiveException.errorCodeAsString`.
(https://github.com/aeron-io/aeron/pull/2094[#2094])
* **[Driver/Java]** Validate first received frame in every packet and reject misformed packets.
* **[Driver/Java]** Validate all frames in the received packet.
* **[Java]** New agreed checkstyle.xml across Aeron repositories and fixes (`MissingOverrides` and `NewLineAtEndOfFile`).
* **[Client/C{plus}{plus} Wrapper]** Avoid allocation for common vectored offers.
(https://github.com/aeron-io/aeron/pull/2098[#2098])
* **[Client/C{plus}{plus} Wrapper]**  Make C{plus}{plus} client compatible with C and Java with respect to empty vectors
being offered to a publication.
* **[Cluster]** Make standby snapshot replicator use archive async connect.
(https://github.com/aeron-io/aeron/pull/2102[#2102])
* **[Client/Driver]** Fix round-robin start duplication after index wrap in client poll and driver send rotation.
(https://github.com/aeron-io/aeron/pull/2107[#2107])
* **[Client/Java]** Access C'n'C file with read only access for `LogInspector` and `DriverTool`.
(https://github.com/aeron-io/aeron/pull/2105[#2105])
* **[Client/C]** Read-only C'n'C file access for C tooling. (https://github.com/aeron-io/aeron/pull/2104[#2104])
* **[Driver/C]** Prevent multicast subcription conflicts with differing multicast addresses and the same port.
(https://github.com/aeron-io/aeron/pull/2106[#2106])
* **[Client/Java]** Update distinct error log message `O != 0`. (https://github.com/aeron-io/aeron/pull/2110[#2110])
* **[Driver/Java]** Change Aeron thread names in Java driver to follow newly established conventions while maintaining
support for old names. (https://github.com/aeron-io/aeron/pull/2109[#2109])
* **[Cluster]** Change Aeron thread names in Cluster to follow newly established conventions.
(https://github.com/aeron-io/aeron/pull/2112[#2112])
* **[Archive/Java]** Change Aeron thread names in Archive to follow newly established conventions while maintaining
support for old names. (https://github.com/aeron-io/aeron/pull/2111[#2111])
* **[Driver/C]** Change thread names in C driver to follow newly established convention in new thread naming mode.
(https://github.com/aeron-io/aeron/pull/2116[#2116])
* **[Driver]** Fixes delay in sending the response setup message. (https://github.com/aeron-io/aeron/pull/2117[#2117])
* **[Logging]** Rewrite of Aeron Logging. (https://github.com/aeron-io/aeron/pull/2115[#2115])
* **[Cluster]** Ensure `ClusterTool` consensus module proxy publication has connected before attempting to use
it. (https://github.com/aeron-io/aeron/pull/2119[#2119])
* **[Client/C/C{plus}{plus}]** Surface errors from the archive as the actual errors from the archive instead of using a
generic error. (https://github.com/aeron-io/aeron/pull/2108[#2108])
* **[Driver/C]** Reduce the command buffer length to `128k` as the previous size (`128m`) was unnecessarily large.
* **[Driver/Java]** Pass packet length into `LossGenerator#shouldDropFrame` calls instead of `frameLength`, i.e. align
behavior with the C driver.
* **[Driver/C]** Validate all frames in a received packet before inserting into a log buffer.
* **[Cluster]** Send a termination request to the ClusteredServiceContainer when the ConsensusModuleAgent is closed.
(https://github.com/aeron-io/aeron/pull/2093[#2093])
* **[Archive]** When scanning a segment file ensure that the correct offset is used with the io length sized buffer.
* **[Archive]** Close `ControlSession` only if the request came from the same as image that was used to create it.
* **[Archive]** Only allow control requests if they come from the same source as the `ControlSession` itself.
* **[Archive]** Enforce max concurrent recordings based on the successful calls to `startRecording/extendRecording`
rather than the number of the `RecordingSessions` which are created lazily when publication connects.
* **[Client/Java]** Move to zero garbage printing of IP addresses for Resolution frames.
* **[Driver]** Adjust strict `SM` validation, i.e. allow positions with half a term behind and up to 1.5 terms ahead of
the `snd-pos`.
* **[Driver/C]** Return early if adding command failed due to full driver command buffer.
* **[Driver/C]** Fix potential log buffer memory leak upon driver termination, i.e. drain pending free log buffer
commands.
* **[Driver/C]** Reduce the command buffer length to 128k as the previous size (128m) was unnecessarily large.
* **[Java]** Upgrade to https://github.com/aeron-io/agrona/releases/tag/2.6.0[Agrona 2.6.0].
* **[Java]** Upgrade to https://github.com/aeron-io/simple-binary-encoding/releases/tag/1.40.1[SBE 1.40.1].
* **[Java]** Upgrade to `Gradle` 9.7.1.
* **[Java]** Upgrade to `ByteBuddy` 1.18.12.
* **[Java]** Upgrade to `JUnit` 6.1.3.
* **[Java]** Upgrade to `Shadow` 9.6.1.
* **[Java]** Upgrade to `Versions` 0.61.0.
* **[Java]** Upgrade to `jackson-dataformat-cbor` 3.2.2.
...
* **[Driver]** Move log buffer creation/removal out of the `conductor` thread.
+
Creating and removing log buffers can take a long time (on the order of hundreds of milliseconds for large term buffers).
Previously this was done on the driver conductor thread which meant a stall whenever such operation was executed. This
stall then was impacting end to end latency of the message flows, because conductor was not updating the stream counters
while stalled (i.e. `pub-lmt`).
+
Now these operation are executed on the background thread (`NativeResourceAgent`) and the driver conductor is no longer
blocked by them.

* **Breaking:** **[Driver]** `AsyncExecutor` configuration changes.
+
`AsyncExecutor` was renamed to `NativeResourceAgent` along side the corresponding configuration options (i.e.
`aeron.driver.async.executor.idle.strategy` was renamed to `aeron.driver.native.resource.agent.idle.strategy`).
+
`NativeResourceAgent` is now tied to the threading mode of the media driver, i.e. it is no longer possible to
enable/disable it separately (`aeron.driver.async.executor.enabled` was removed). When thread mode is configured to:
+
--
** `DEDICATED`/`SHARED_NETWORK` - it will run on a dedicated thread.
** `SHARED`/`INVOKER` - it will run on the `conductor` thread.
--
+
Thread name was changed from `aeron-executor` to `aeron-md-nra`.

* **[Archive]** Truncating a recording now stops any concurrent replays of that recording before proceeding with the
deletion of segments.

* **[Event Logging]** The event debug logging when writing to a file allows for files to be rolled automatically based
on the max configured file size (`aeron.event.log.file.max.length`). By default max file size is unlimited, i.e. file
rollover is disabled.

* **Breaking:** **[Cluster]** Removed unused method `ConsensusModuleExtension#supportedSchemaId()` that gave the
impression as if there could only be a single schema supported by an extension. This is a breaking change in an
internal API.

* **Breaking:** **[Cluster]** Introduce `VersionValidator` interface and use it within the `ConsensusModule` and
`ClusteredServiceContainer` in place of the `AppVersionValidator` allowing for custom versions. `AppVersionValidator`
remains the default.

* **[Driver/C]** CPU affinity for driver threads is now cgroup aware. The user supplied CPU number for each thread can
be an offset into the CPU set for the processes current CPU group.

* **[Driver/C]** CPU affinity is now supported for the background thread used for longer operations, e.g. name
resolution.

* **Breaking:** **[Driver/C]** `aeron_spsc_rb_writev` was removed.

* **[Client]** Image `controlledPeek` APIs are deprecated for removal and will be removed in the next release
(i.e. *1.53.0*):
**  `io.aeron.Image.controlledPeek` and `io.aeron.Image.position(long)`.
** `aeron_image_controlled_peek` and `aeron_image_set_position`.
** `aeron::Image::controlledPeek` and `aeron::Image::position`.
...
* **[Driver/C]** CPU affinity for the background thread (`aeron.driver.native.resource.agent.cpu.affinity`) is not
applied correctly when set, i.e. it ends up sharing the same core as the `conductor` thread.
+
Affects <<1.52.0 (2026-07-03)>> and <<1.52.1 (2026-07-08)>>.
+
Fixed in <<1.52.2 (2026-07-10)>>.

* **[Driver]** Sender thread busy spins even when idle. (https://github.com/aeron-io/aeron/issues/2092/[#2092])
+
Affects <<1.52.0 (2026-07-03)>> and <<1.52.1 (2026-07-08)>>.
+
Fixed in <<1.52.2 (2026-07-10)>>.
...
* **[Driver/C]** Set default `resolver_bootstrap_neighbor_resolution_interval` correctly. (https://github.com/aeron-io/aeron/issues/2035[#2035])
* **[Client]** Refresh the recording descriptor before attempting the replay again if a Persistent Subscription loses its
image during a replay. (https://github.com/aeron-io/aeron/pull/2037[#2037])
* **[Client]** Handle error condition in PersistentSubscription where live position goes backwards. (https://github.com/aeron-io/aeron/pull/2040[#2040])
* **[C]** Improved support for AddressSanitizer builds and testing on Windows. (https://github.com/aeron-io/aeron/pull/2022[#2022])
* **[Client/Java]** Fixed potential integer overflow of `blockLengthLimit` in `Image.blockPoll` and `Image.rawPoll`. (https://github.com/aeron-io/aeron/pull/2036[#2036])
* **[Client/C]** Remove `aeron_archive_persistent_subscription.h` from the package as it has no public API.
* **[Client/C]** Fixed issue where conductor command drain loop might break out early when there was a padding record
in the queue. (https://github.com/aeron-io/aeron/pull/2045[#2045])
* **[C/C{plus}{plus} Wrapper]** Add PowerPC 64-bit (`ppc64le`) support to C and C++ clients. (https://github.com/aeron-io/aeron/pull/2060[#2060])
* **[Cluster]** Added `VersionValidator` interface. (https://github.com/aeron-io/aeron/pull/2059[#2059])
* **[Driver/C]** Fix `session-id` checks for network publications.
* **[Driver/C]** Remove `aeron_spsc_rb_writev` and restore compilation on Windows.
* **[Driver]** Do not re-resolve control/endpoint addresses if the corresponding endpoint is not active (i.e. closing).
* **[Driver]** Fix a race condition when there are only spies and spies simulate connection, i.e. adding/removing spies
mutated the array that the sender thread was iterating over.
* **[Driver/Java]** Remove `INIT_IN_PROGRESS` stream interest marker if `PublicationImage` creation fails, i.e. allow
image creation to be retried.
* **[Driver/C]** Do not apply `IMAGE_COOL_DOWN` if EOS was received (i.e. align with `DataPacketDispatcher`).
* **[C/Archive Client]** Filter signal events by session when polling for recording descriptors so only signals for the
client's session are seen.
* **[C/Archive Client]** Correctly pass the `UNKNOWN_REPLICATION` error code when polling for responses that allow for
a specific error.
* **[Archive]** Retry the creation of the control subscription when a temporary failure occurs.
* **[Archive]** Retry creation of the replay publication.
* **[Archive]** Show `clientInfo` of the `CMD_IN_AUTH_CONNECT` agent logging event.
* **[Java]** Upgrade to https://github.com/aeron-io/agrona/releases/tag/2.5.0[Agrona 2.5.0].
* **[Java]** Upgrade to https://github.com/aeron-io/simple-binary-encoding/releases/tag/1.39.0[SBE 1.39.0].
* **[Java]** Upgrade to `Gradle` 9.6.1.
* **[Java]** Upgrade to `JUnit` 6.1.1.
* **[Java]** Upgrade to `ByteBuddy` 1.18.11.
* **[Java]** Upgrade to `Shadow` 9.4.3.
* **[Java]** Upgrade to `ASM` 9.10.1.
* **[C]** Upgrade to `HdrHistogram` 0.11.10.
* **[C]** Upgrade to `CMake` 4.3.4.
...
in the queue. (https://github.com/aeron-io/aeron/pull/2045[#2045])
* **[Client/C]** Remove `aeron_archive_persistent_subscription.h` from the package as it has no public API.
* **[Driver/C]** Use `EBUSY` error code when active media driver is detected.
* **[Client/C]**  Fix a race condition in client conductor. (https://github.com/aeron-io/aeron/pull/2043[#2043])
* **[Archive]** Show `clientInfo` of the `CMD_IN_AUTH_CONNECT` agent logging event.
...
* **[Archive]** Added `PersistentSubscriptions` to Aeron Archive for all languages (Java, C++ and C).
+
https://github.com/aeron-io/aeron/wiki/Persistent-Subscriptions[Persistent Subscriptions] is an alternative to
ReplayMerge with IPC and Spy support, plus a fallback mechanism for slow consumers.

* **Breaking:** **[Driver]** Async executor is now a single-threaded `Agent`.
+
Previously async executor could be configured to run on `N` threads (Java only). Now it can either be enabled in which
case it runs on a dedicated `aeron-executor` thread or disabled (i.e. runs on the `conductor` thread instead).
+
Removed the `aeron.driver.async.executor.threads` property and the corresponding
configuration API (i.e. `MediaDriver.Context.asyncTaskExecutorThreads(int)`,
`MediaDriver.Context.asyncTaskExecutor(Executor)`).
+
Added new configuration properties:
+
** `aeron.driver.async.executor.enabled` - to enable/disable async executor (defaults to `true`).
** `aeron.driver.async.executor.idle.strategy` - to set idle strategy for an async executor when enabled. Defaults to
sleeping for 1ms when idle.

* **Breaking:** **[Java Driver]** The name resolver interface has been changed. The `NameResolver` has had the lifecycle
methods (`init`/`doWork`) removed to narrow the interface to the resolution methods. A new interface `NameResolverAgent`
has been introduce to include the `Agent` lifecycle methods. Custom name resolvers will need to implement this interface
to be managed correctly by the driver and be passed into the `MediaDriver.Context`. Default methods are provided so
implementing is not required.

* **Breaking:** **[Driver/C]** The name resolver interface has been changed. The C name resolver now requires an
additional `on_start` lifecycle method.

* **Breaking:** **[Driver/C]** The `aeronmd` binary now requires the aeron client library (`libaeron.so`/`aeron.dll`)
to be present when running. This is because previously the library code was included twice.  Once in the driver library
(`libaeron_driver.so`/`aeron_driver.dll`) and once in executable. The logging relies on a thread local symbol and without
this change it could be duplicated in some situations. The client library should reside in the same location as the
driver library when being deployed.

* **Breaking:** **[Cluster]** The classes `RecordingLog.Snapshot`, `RecordingLog.Log` and `Recording.RecoveryPlan` have
been changed from flat classes to records, so the data accessors are now methods and not fields.
...
* **Breaking:** **[Client/C]** Removed `async` parameter from the `aeron_on_new_publication_t` and
`aeron_async_add_subscription_t` callbacks.
+
NOTE: These callbacks should only be used to signal resource availability, i.e. the polling of the async struct should
be done on the application thread.
...
* poll call (e.g. `aeron_async_add_subscription_poll`) completes, i.e. resource is created, errored or timed out.
* resource creation is canceled without polling (e.g. `aeron_async_add_subscription_cancel`).
* resource removal operation is invoked without polling (e.g. `aeron_async_remove_subscription`).
* `aeron_client_t` is closed.
====

* **Breaking:** **[Client/Java]** Removed `registrationId` parameter from the `io.aeron.Counter` constructor.

* **[Breaking]**: **[Driver/Java/C]** Reject SMs that out of bounds, i.e. whose position is more than half a term
ahead or behind of the sender position. (https://github.com/aeron-io/aeron/pull/1971[#1971])

* **[Breaking]**: **[Client/C{plus}{plus} Wrapper]** Remove `close()` method from `Publication/ExclusivePublication`,
i.e. the underlying `aeron_publication_t/aeron_exclusive_publication_t` will be closed when the corresponding C{plus}{plus}
object is destroyed.

* **[Client/C]**: Added API to cancel async add operations:
** `aeron_async_add_subscription_cancel`
** `aeron_async_add_publication_cancel`
** `aeron_async_add_exclusive_publication_cancel`
** `aeron_async_add_counter_cancel`

* **[Client/C]**: Added API to remove resources using their registration IDs:
** `aeron_async_remove_subscription`
** `aeron_async_remove_publication`
** `aeron_async_remove_exclusive_publication`
** `aeron_async_remove_counter`

=== Known issues

* [Driver/C] Bootstrap neighbor resolution is disabled out of the box due to default value not being set.
+
TIP: The workaround is to explicitly set `aeron.driver.resolver.bootstrap.neighbor.resolution.interval` explicitly, e.g.
`-Daeron.driver.resolver.bootstrap.neighbor.resolution.interval=10s` or
`AERON_DRIVER_RESOLVER_BOOTSTRAP_NEIGHBOR_RESOLUTION_INTERVAL=10s`.
+
NOTE: The issue was fixed in <<1.51.1 (2026-06-30)>>.

=== Changelog
* **[C]** Removed `aeron_async_cmd_free` API, i.e. free async structs automatically when:
** poll call completes, i.e. resource is created, errored or timed out.
** resource creation is canceled without polling.
** resource removal operation is invoked without polling.
** `aeron_client_t` is closed.
* **[C]** Try to resolve host name as IPv4 first before attempting an IPv6 resolution. (https://github.com/aeron-io/aeron/issues/2020[#2020])
* **[C]** Align C build output on Windows with the Unix build.
* **[C]** Added `AERON_RB_IS_CAPACITY_VALID` upper bound check. (https://github.com/aeron-io/aeron/pull/2017[#2017])
* **[C]** Drain pending remove cmds on conductor close to avoid a possible leak. (https://github.com/aeron-io/aeron/pull/2008[#2008])
* **[C]** Fix bug `aeron_spsc_rb try_claim` when full. (https://github.com/aeron-io/aeron/pull/2015[#2015])
* **[C]** Async executor agent refactoring. (https://github.com/aeron-io/aeron/pull/2014[#2014])
* **[CI]** Add a once per day push to the maven central snapshot repository.
* **[Driver/Java/C]** Driver name resolver configuration. (https://github.com/aeron-io/aeron/pull/2013[#2013])
* **[Driver/C]** Align Driver Name Resolver with Java implementation. (https://github.com/aeron-io/aeron/pull/2012[#2012])
* **[C]** Add pthread-style TLS destructor support on Windows. (https://github.com/aeron-io/aeron/pull/2009[#2009])
* **[Driver/C]** Fix misleading SO_SNDBUF reference in rcvbuf validation error message. (https://github.com/aeron-io/aeron/pull/2011[#2011])
* **[Driver/C]** Add per-bootstrap neighbor counters. (https://github.com/aeron-io/aeron/pull/2010[#2010])
* **[Java]** Make `AsyncExecutor` an `Agent`, i.e. remove executor thread pool and replace with a single-threaded agent.
Move ownership of the `NameResolver` to `AsyncExecutor` to make all access to `NameResolver` single-threaded.
* **[Checkstyle]** Allow inline return tags in the Javadoc comments.
* **[C]** implement `aeron_image_release`. (https://github.com/aeron-io/aeron/pull/2005[#2005])
* **[C]** Fixed leak of async connect commands if connection was closed before completing. (https://github.com/aeron-io/aeron/pull/2007[#2007])
* **[C]** Fixed possible leak of ClientConductorIsLengthSufficientTest test parameters when the test is skipped. (https://github.com/aeron-io/aeron/pull/2006[#2006])
* **[Java]** Refactor `Counter` class, i.e. define both `registrationId` and `correlationId` to allow for better handling
of the static counters.
* **[C/Client]** Make static counter creation operation cancellable. Use a dedicated resource type
`AERON_CLIENT_MANAGED_RESOURCE_TYPE_STATIC_COUNTER` for static counters. Add `correlation_id` to
`aeron_counter_constants_t`. Validate async resource type before cancelling an operation.
* **[Driver/Java]** Add per-bootstrap neighbor counters. (https://github.com/aeron-io/aeron/pull/2004[#2004])
* **[Driver/Java]** Update DriverNameResolver to align more with C code. (https://github.com/aeron-io/aeron/pull/1995[#1995])
* **[C/Client]** Removed `async` parameter from the `aeron_on_new_publication_t` and `aeron_async_add_subscription_t` callbacks.
* **[Driver/Java]** Separate NameResolver interface and create NameResolverAgent interface. (https://github.com/aeron-io/aeron/pull/2001[#2001])
* **[C]** Fix use-after-free issue when removing pending close resource fails to submit delete command to the media driver,
i.e. perform a single shot close action just like a normal close command would.
* **[C]** Correctly decode errors in aeron_delete_file on Windows.
* **[PS1]** Update the configuration of the VsDevShell in the cppbuild.ps1 script.
* **[Driver/C]** Introduce a start life cycle for name resolvers.
* **[C]** aeron_idle_strategy_backoff_state_init_args error message fix. (https://github.com/aeron-io/aeron/pull/2000[#2000])
* **[C]** Added check for `!is_closed` in poll functions to prevent potentially accessing free'd memory. (https://github.com/aeron-io/aeron/pull/1998[#1998])
* **[C]** Notify `on_complete` callbacks for pending close operations when client conductor is closed.
* **[C{plus}{plus} Wrapper]** Rely on the guaranteed close of the underlying `aeron_subscription_t` to call the `on_close` callback to
free the memory.
* **[C]** Make `AERON_CLIENT_ERROR_` codes positive to match the rest of Aeron-specific error codes. Handle client error
codes when setting errors, i.e. write user-friendly message.
* **[C]** Fix `AERON_PUBLICATION_LINK_INIT` wrong var names. (https://github.com/aeron-io/aeron/pull/1999[#1999])
* **[C]** Return early if `aeron_driver_name_resolver_t` allocation fails.
* **[Driver/C]** Change driver name resolver to all defined bootstrap neighbors rather than one at a time.
* **[Driver/C]** Add configuration option for bootstrap neighbor resolution interval.
* **[C]** Ensure that `aeron_subscription_t/aeron_publication_t/aeron_exclusive_publication_t/aeron_counter_t` resources
are closed even if client conductor buffer is full.
* **[C]** Use `AERON_CLIENT_ERROR_DRIVER_BUFFER_FULL` error code when failing to add to the driver command queue.
* **[C]** Fixed `aeron_driver_receiver_on_remove_destination` error message. (https://github.com/aeron-io/aeron/pull/1997[#1997])
* **[C]** Fix `aeron_diver_uri_publication_params` term_offset. (https://github.com/aeron-io/aeron/pull/1996[#1996])
* **[Java]** Always perform reentrancy checks when removing Subscription/Publication.
* **[Driver/Java]** When re-resolving bootstrap addresses, resolve all of them at once.
* **[Driver/Java]** Driver Name Resolution, publish self name/address frames to all bootstrap neighbors. (https://github.com/aeron-io/aeron/pull/1993[#1993])
* **[C]** More hashmap allocation cleanups.
* **[C]** Fixes `aeron_array_to_ptr_hash_map_rehash` memory leak. (https://github.com/aeron-io/aeron/pull/1992[#1992])
* **[C]** Fix `aeron_receive_channel_endpoint_elicit_setup work_count` bug. (https://github.com/aeron-io/aeron/pull/1991[#1991])
* **[C]** Fix `use-after-free` bug whereby an async resource could be deleted before
`on_new_publication/exclusive_publication/subscription` was invoked, because the resource was released too early.
* **[C]** Handle `aeron_broadcast_receiver_receive` errors and hide OS errors when invoking `error_handler`, i.e. only error
codes defined by Aeron will be passed to the handler and OS error codes will be replaced with `AERON_ERROR_CODE_GENERIC_ERROR`.
* **[C]** Use common error handling to delete partially created `aeron_send_channel_endpoint_t`.
* **[C]** Prevent segfault when deleting a partially constructed `aeron_receive_channel_endpoint_t`.
* **[C]** Fixes aeron_receive_channel_endpoint_create memory leak. (https://github.com/aeron-io/aeron/pull/1990[#1990])
* **[C]** Allow `aeron_data_packet_dispatcher_close` to be called even if `aeron_data_packet_dispatcher_init` fails.
* **[C]** Fix DNR config parsing default values.
* **[Driver/C]** Implement additional configuration options for the driver name resolver.
* **[C]** Fixes `aeron_udp_destination_tracker_manual_add_destination` wrong arg order. (https://github.com/aeron-io/aeron/pull/1989[#1989])
* **[Driver/C]** Use `CMSG_LEN` correctly for `recvmsg/recvmmsg`.
* **[Driver/Java]** Make `DriverConductor#onAddSendDestination` and `DriverConductor#onRemoveSendDestination` use async DNS.
(https://github.com/aeron-io/aeron/pull/1987[#1987])
* **[Archive]** `ArchiveException` build error messages for persistent subscriptions. (https://github.com/aeron-io/aeron/pull/1986[#1986])
* **[C]** Add API for cancelling async add operations and removing by registration IDs. (https://github.com/aeron-io/aeron/pull/1974[#1974])
* **[Driver/Java]** Implement additional configuration options for the driver name resolver.
* **[Archive]** Added `INVALID_POSITION` error code. (https://github.com/aeron-io/aeron/pull/1980[#1980])
* **[C]** Fix out of bounds access issue when setting client name of the max allowed size (i.e. `AERON_COUNTER_MAX_CLIENT_NAME_LENGTH`).
* **[C]** Truncate thread name if longer than 15 characters due to `pthread_setname_np` limit on Linux.
* **[Driver/C]** Moved some client source files out of the `aeron_driver` source list and linked `aeron_driver` to `libaeron`
to use its copy of the source files instead. (https://github.com/aeron-io/aeron/pull/1972[#1972])
* **[C]** Improved Windows implementation of `aeron_dlopen/aeron_dlsym`. (https://github.com/aeron-io/aeron/pull/1976[#1976])
* **[C]** `aeron_connect` for MSCV unwanted printf. (https://github.com/aeron-io/aeron/pull/1979[#1979])
* **[C]** Fix `aeron_udp_channel_transport_sendv` NULL address dereference. (https://github.com/aeron-io/aeron/pull/1982[#1982])
* **[C]** `aeron_udp_channel_transport_init` `recv_fd` init bug. (https://github.com/aeron-io/aeron/pull/1978[#1978])
* **[C]** Fixes a memory leak in the `aeron_udp_channel_transport_close`. (https://github.com/aeron-io/aeron/pull/1977[#1977])
* **[C/C{plus}{plus} Wrapper]** Added support for C/C{plus}{plus} builds using MSVC on Windows ARM. (https://github.com/aeron-io/aeron/pull/1975[#1975])
* **[Client/Java/C]** Use `aeron-client` as a name for client conductor thread, i.e. shorter name to fit the 15 characters
limit of the `pthread_setname_np` API.
* **[Java]** Align exception messages across C and Java.
* **[Driver/C]** Set error status for failed CnC file checks instead just logging them.
* **[C]** Return length of the channel in case no wildcard port is set on the subscription.
* **[Driver/C]** Socket can leak with multicast. (https://github.com/aeron-io/aeron/pull/1973[#1973])
* **[C]** Fallback to env variables for `JAVA_HOME` and friends during build.
* **[Java/C]** Strict SM handling. (https://github.com/aeron-io/aeron/pull/1971[#1971])
* **[Cluster]** Recording log validator. (https://github.com/aeron-io/aeron/pull/1969[#1969])
* **[Cluster]** Fixed newline formatting in cluster tool help text. (https://github.com/aeron-io/aeron/pull/1968[#1968])
* **[Cluster]** Rollback the consensus module leader's next committed session id. (https://github.com/aeron-io/aeron/pull/1918[#1918])
* **[C]** Handle `AERON_CLIENT_ERROR_BUFFER_FULL` when sending driver termination command.
* **[C]** Sanitise build options are compiler-dependent.
* **[Archive]** Correct warning for `compact` command.
* **[C{plus}{plus} Wrapper]** Fix forward definition of the `Aeron` class and reorder constructor arguments.
* **[C{plus}{plus} Wrapper]** Remove `close()` method from `Publication/ExclusivePublication`, i.e. the underlying C publications wil
 be closed when the corresponding C{plus}{plus} object is destroyed.
* **[C{plus}{plus} Wrapper]** Fix pending `AsyncDestination` memory leak when not polled to completion.
* **[C{plus}{plus} Wrapper]** Handle a case when a media driver dies/is stopped leading to the underlying C-managed resources to being
closed, i.e. manually delete `AsyncAddSubscription` instance since no `on_close_complete` callback will be executed in
this case.
* **[C{plus}{plus} Wrapper]** Ensure that `Aeron` instance is deleted after all of its children are deleted. Deleting `Aeron` before
its children leads to a segfault due to use-after-free of C-managed memory.
* **[C{plus}{plus} Wrapper]** Remove internal locks from `ExclusivePublication` and `Subscription`.
* **[C]** Do not block when adding/removing handlers if client conductor was closed (i.e. when media driver terminates).
* **[C]** Fix blocking logic of the `aeron_add_xxx_handler/aeron_remove_xxx_handler` methods, i.e. do not exit early if
client command queue is empty and instead retry until command is added and confirmed or timeout occurs.
* **[C{plus}{plus} Wrapper]** Free `AsyncAddSubscription` associated with a `Subscription` when it is destroyed. Ensure that any
dangling `AsyncAddSubscription` instances are freed by the `Aeron` when it is destroyed, i.e. if
`aeron_subscription_close` didn't run (because `aeron_client` was closed).
* **[C]** Close image before invoking unavailable handler when closing subscription.
* **[Driver/C]** Avoid double freeing memory when setting new idle strategy programmatically.
* **[C{plus}{plus} Wrapper]** Set idle strategy on the underlying `aeron_context` using `m_idleSleepDuration`.
* **[C]** Allow setting idle strategy on the `aeron_context`.
* **[Cluster]** Update SnapshotReplication to allow accessing snapshot currently being replicated.
* **[Cluster]** Add better message for `ClusteredServiceAgent` startup exception.
* **[Cluster]** Expose package private `replicationId` in `SnapshotReplication`.
* **[Cluster]** `ConsensusModuleAgent` idle strategy usage. (https://github.com/aeron-io/aeron/pull/1959[#1959])
* **[Cluster]** Allow a delay before processing standby snapshot notifications.
* **[Driver/C]** Fix memory leak whereby async commands submitted to the async executor were not freed upon driver termination.
(https://github.com/aeron-io/aeron/pull/1960[#1960])
* **[Java]** Fix all Javadoc warnings produced by JDK24.
* **[Cluster]** Delay processing of standby snapshot notifications until after the commit position has passed the snapshot
log position.
* **[Cluster]** Use `ConsensusModule#acceptStandbySnapshots` to reject standby snapshot notifications if node is not
configured for accepting standby snapshots. (https://github.com/aeron-io/aeron/pull/1957[#1957])
* **[C]** Remove `aeron_executor_task_do_complete` and simplify interactions when return queue is not used.
* **[C]** Remove explicit node management from `aeron_linked_queue`.
* **[C]** Use agent runner to implement `aeron_executor`.
* **[C]** Fix agent runner state assignment.
* **[Cluster]** Add `isStandby` to `Join` Log message.
* **[Cluster]** Cluster backup to ask for latest snapshot.
...
* **[Cluster]** Backup query responses are now resolved using the most recent entry in the `RecordingLog` instead of
being derived from the `RecoveryPlan`.
* **[Cluster]** When a `ClusteredServiceAgent` fails to start, e.g. from an exception during snapshot load, the
`ConsensusModuleAgent` will also terminate with an error in the log. Previously this would hang indefinitely.
(https://github.com/aeron-io/aeron/pull/1963[#1963])
* **[Samples/Java]** Added a sample for ReplayMerge.  (https://github.com/aeron-io/aeron/pull/1956[#1956])
...
* **[Cluster]** Now retries when a `RESOURCE_TEMPORARILY_UNAVAILABLE` error is encountered when creating an egress subscription.
...
* **[Java]** Upgrade to https://github.com/aeron-io/simple-binary-encoding/releases/tag/1.38.1[SBE 1.38.1].
* **[Java]** Upgrade to `Gradle` 9.5.0.
* **[Java]** Upgrade to `ByteBuddy` 1.18.8.
...
* **[C]** Upgrade to `CMake` 4.3.2.

== 1.50.5 (2026-09-02)

* **[Driver/Java]** Fix a race condition when there are only spies and spies simulate connection, i.e. adding/removing
spies mutated the array that the sender thread was iterating over.
* **[Driver/C]** Fix a race condition when there are only spies and spies simulate connection, i.e. adding/removing
spies mutated the array that the sender thread was iterating over.
...
WARNING: **DO NOT USE: Not all artifacts were released!**
...
* **[Driver/C]** Expose try_connect_stream method for use from within ATS.
* **[Driver/C]** Remove unconnected stream message.
* **[Driver/C]** Fix string format for AERON_SET_ERROR arguments.
...
* **[Driver/C]** Fixes for fixed loss interceptor used for testing.
...
* **[Driver/C]** Handle a case when `getifaddrs` call fails.
...
* **[Java/Driver/C]** Support configuration flag for experimental features (#1577)
* **[Driver/C]** Fix buffer overflow when updating counter labels.
...
* **[Driver/C]** async DNS resolution for the C media driver (#1566)
...
* **[Driver/C]** Add `aeron_msync` and use to flush CnC file contents upon start and shutdown.
...
* **[Driver/C]** Install pre-/post-interceptors logging interceptors if the incoming/outgoing chains are not NULL. Change logging order for outgoing events to log before executing command to match Java and log plain messages before encryption.
* **[Java Driver]** Fix `FRAME_IN/FRAME_OUT` event logging, i.e. allow redefinition of previously loaded classes.
* **[Driver/C]** Scope logging functions with a "log" struct. (#1556)
* **[Driver/C]** Add C driver logging for send_nak_message and resend (#1553)
* **[Driver/C]** Track pending setups for cases when notified of unconnected messages. Only allow pending setups for unknown interest or existing images when in ACTIVE state. Handle unconnected streams by sending setup eliciting SMs.
* **[Driver/C]** Fix potential leak in when adding stream interest to subscription by session.
* **[Java Samples]** Improve response channels samples.
* **[Driver/C]** Align min flow control implementation with Java + allocate memory only if a receiver is added.
...
* **[Driver/C]** Align implementation of retransmit handler with Java driver.
...
* **[Driver/C]** Per stream nak delay (#1570)
...
* **[Driver/C]** check for response parameters when adding destinations (#1565)
...
* **[Driver/C]** Add reference id to counters.
...
* **[Driver/C]** Put enum events in order.
* **[Driver/C]** Remove reliance of max event num in enum for driver agent.
* **[Driver/C]** Fix signed comparisons.
* **[Driver/C]** Declare void param.
* **[Driver/C]** Keep loop types consistent.
...
* **[Driver/C]** Fix capture of spy subscription channels so that image availability callbacks include the correct subscription URI.
...
* **[Driver/C]** Introduce a parameters structure that can be used to carry transport configuration information when initialising a udp_channel_transport. Also pass MTU information through the bindings to allow for improved validation of within the DPDK bindings.
...
* **[Driver/C]** Ensure loop iteration index is used when setting timestamps for publication.
...
* **[Driver/C]** Implement low storage warnings for the C driver.
...
* **[Driver/C]** add max_retransmission_length.
...
* **[Driver/C]** Fix error reporting for aeron_port_manager.
* **[Driver/C]** Implement timestamp capturing for the recvmsg path.
* **[Driver/C]** Switch sender poller to poll for a single SM, i.e. do the `recvmsg` syscall.
* **[Driver/C]** Add additional validation for frame_length on status messages.
* **[Driver/C]** Fix error format strings when configuring sockets for the udp transport.
...
* **[Driver/C]** Use correct type for array length calculation, fix #1539.
...
* Ignore resting subscription positions when calculating join position or draining driver entities when tethering is being used (Driver/C).
...
* Do not fail if Aeron directory exists, but the CnC file does not (Driver/C).
* Use `aeron_errcode` to get the latest error code when reporting problems creating the Aeron directory (Driver/C).
...
* Move close operations on MDS transports on the conductor not the receiver thread (Driver/C).
...
* Fix CMD_OUT_ERROR log format (Driver/C).
* Fix `aeron_err_clear()` (C).
* C driver handle untethered subscriptions correctly, ensure that tethering behaviour matches Java driver (Driver/C).
...
* Reset eos_flagged when adding a receiver (Driver/C)
* Filter images by session id when adding new network subscriptions (Driver/C)
