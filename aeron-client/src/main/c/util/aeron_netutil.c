...
int aeron_parse_named_interface(const char *interface_str, aeron_named_interface_t *out)
{
    ...
    {
        AERON_APPEND_ERR("name length %d out of bounds", name_len);
        return -1;
    }
    ...
    if (idx < len)
    {
        ...
        {
            AERON_APPEND_ERR("%s", "unexpected character after name closing brace");
            return -1;
        }
        ...
        {
            AERON_APPEND_ERR("port length %d out of bounds", port_len);
            return -1;
        }
    ...
    int family, const char *interface_str, struct sockaddr_storage *if_addr, unsigned int *if_index)
{
    ...
    if (aeron_parse_named_interface(interface_str, &named_interface))
    {
