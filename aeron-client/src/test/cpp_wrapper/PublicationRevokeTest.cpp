...
TEST_P(PublicationRevokeTest, revokeTestConcurrent)
{
    ...
    std::int64_t pub2Id = aeron->addPublication(publicationChannel, streamId);

    POLL_FOR_NON_NULL(sub, aeron->findSubscription(subId), invoker);
    POLL_FOR_NON_NULL(pub, aeron->findPublication(pubId), invoker);
    POLL_FOR_NON_NULL(pub2, aeron->findPublication(pub2Id), invoker);
    ...
    std::int32_t pubPosCounterId = CountersReader::NULL_COUNTER_ID;
    aeron->countersReader().forEach(
        ...
        {
            if (CountersReader::NULL_COUNTER_ID == pubPosCounterId &&
                AeronCounters::DRIVER_PUBLISHER_POS_TYPE_ID == counterTypeId &&
                label.find(publicationChannel) != std::string::npos)
            {
                pubPosCounterId = counterId;
            }
        });
    ...
    POLL_FOR(CountersReader::RECORD_RECLAIMED != aeron->countersReader().getCounterValue(pubPosCounterId), invoker);
